...
def initialize_kmer_df(k):
    """
    初始化一个包含所有 k-mer 的 DataFrame，其中每个 k-mer 的计数都为 0。
    参数：k -- k-mer 的长度
    返回值：一个 DataFrame，其中包含所有 k-mer 的计数
    """
    # 生成 k-mer 列表
    kmers = []
    for i in range(4 ** k):
...
def count_kmers_in_df(df, sequence, k):
    """
    将给定 DNA 序列中的 k-mer 计数添加到 DataFrame 中。
    参数：df -- 一个 DataFrame，包含所有 k-mer 及其计数 sequence -- DNA 序列 k -- k-mer 的长度
    返回值：无
    """
    # 统计 k-mer 的计数
    kmer_counts = {}
    ...
    with open(fn) as f:
        for r in f:
            ...
            if (tokens[0] != tokens[3]) or (tokens[0] == tokens[3]
                ...
                # 打印结果
                #print('left_count_list:',left_count_list)
